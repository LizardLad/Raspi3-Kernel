...
static void uint_to_string(unsigned long num, char *str, unsigned int base, bool alt_digits);
...
default:
			...
			if(base != 0)
			{
				...
				uint_to_string(ularg, &str[0], base, alt_digits);
...
int snprintf(char *buf, size_t bufSize, const char *fmt, ...)
{
	...
	char *s = buf;					// Hold initial buffer pointer
	output_count = 0;				// Zero output count
	max_out_count = (bufSize > 0) ? bufSize-1 : 0;	// This is a maximum size of output
	va_start(args, fmt);				// Create argument list
	_doprnt(fmt, args, prn_to_buf, (void*)&s);	// Run conversions
	va_end(args);					// Done with argument list
	*s = '\0';					// Add ascii terminator
...
int vprintf(const char* fmt, va_list arg)
{
	...
	if(console_write_char)
	{
		output_count = 0;						// Zero output count
		max_out_count = INT_MAX;						// This is a maximum size function
		count = _doprnt(fmt, arg, prn_to_func, console_write_char);	// Do the output
	}
...
int vsnprintf(char *buf, size_t buf_size, const char *fmt, va_list args)
{
	char *s = buf;						// Hold initial buffer pointer
	output_count = 0;					// Zero output count
	max_out_count = (buf_size > 0) ? buf_size - 1 : 0;	// This is a maximum size of output
	_doprnt(fmt, args, prn_to_buf, (void*)&s);		// Run conversions
	*s = '\0';						// Add ascii terminator
	return s - buf;						// Return number of characters put in buffer
}
